...
var mui = (function(document, undefined) {
	...
	/**
	 * 將 fn 快取一段時間后, 再被呼叫執行
	 * 此方法爲了避免在 ms 段時間內, 執行 fn 多次. 常用於 resize , scroll , mousemove 等連續性事件中;
	 * 當 ms 設定為 -1, 表示立即執行 fn, 即和直接呼叫 fn 一樣;
	 * 呼叫返回函式的 stop 停止最後一次的 buffer 效果
	 * @param {Object} fn
	 * @param {Object} ms
	...
	$.buffer = function(fn, ms, context) {
		...
		return $.extend(function() {
			if (
				(!lastStart) || // 從未執行過
				(lastEnd >= lastStart && $.now() - lastEnd > ms) || // 上次執行成功后已經超過ms毫秒
				(lastEnd < lastStart && $.now() - lastStart > ms * 8) // 上次執行或未完成，后8*ms毫秒
			) {
...
/**
 * 僅提供簡單的on，off(僅支援事件委託，不支援目前元素繫結，目前元素繫結請直接使用addEventListener,removeEventListener)
 * @param {Object} $
 */
(function($) {
	 ...
	 * @returns {undefined}
	 */
	$.fn.on = function(event, selector, callback) { //僅支援簡單的事件委託,主要是tap事件使用，類似mouse,focus之類暫不封裝支援
	...
	$.fn.off = function(event, selector, callback) {
		return this.each(function() {
			...
			if (delegates[_mid]) {
				//如果off掉了所有當前element的指定的event事件，則remove掉目前element的delegate回撥
...
(function($, window, document) {
	...
	window.addEventListener('click', function(event) { //解決touch與click的target不一致的問題(比如鏈接邊緣點選時，touch的target為html，而click的target為A)
		var target = event.target;
		var isFound = false;
		for (; target && target !== document; target = target.parentNode) {
			if (target.tagName === 'A') {
...
(function($, window) {
	...
	window.addEventListener($.EVENT_CLICK, function(e) {
		//TODO 應該判斷目前target是不是在targets.popover內部，而不是非要相等
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			...
			case $.EVENT_MOVE:
				...
				//修正direction,可在session期間自行鎖定拖拽方向，方便開發scroll類不同方向拖拽外掛巢狀
	...
	$.addInit({
		...
		handle: function() {
			...
			if (!$.os.plus && subpages.length) {
				//暫時只處理單個subpage。後續可以考慮支援多個subpage
...
(function($) {
	var defaultOptions = {
		...
		preloadLimit: 10, //預載入視窗的數量限制(一旦超出，先進先出)
		keyEventBind: {
			backbutton: true,
			menubutton: true
		},
	...
	/**
	 * 打開新視窗
	 * @param {string} url 要打開的頁面地址
	 * @param {string} id 指定頁面ID
	 * @param {object} options 可選:參數,等待,視窗,顯示配置{params:{},waiting:{},styles:{},show:{}}
	 */
	$.openWindow = function(url, id, options) {
		...
		if(webview) { //已快取
			//每次show都需要傳遞動畫參數；
			//預載入的動畫參數優先順序：openWindow配置>preloadPages配置>mui預設配置；
			...
			if(nShow.autoShow) {
				...
				//titleUpdate觸發時機早於loaded，更換為titleUpdate后，可以更早的顯示webview
				webview.addEventListener(nShow.event, showWebview, false);
				//loaded事件發生后，觸發預載入和pagebeforeshow事件
	...
	$.openWindowWithTitle = function(options, titleConfig) {
		...
		if(webview) { //已快取
			...
			if(titleConfig) { //處理原生頭
				...
				view.drawRect($.options.titleConfig.backgroundColor); //繪製背景色
				...
				if(_backImage.base64Data || _backImage.imgSrc) {
					...
					if(_backImage.base64Data) { //優先base64編碼字串
	...
	$.createWindow = function(options, isCreate) {
		...
		if(options.preload) {
					...
					if(options.subpages) {
						$.each(options.subpages, function(index, subpage) {
							...
							if(subpageId) { //過濾空對像
								...
								if(!subWebview) { //如果該webview不存在，則建立
			...
			//TODO 理論上，子webview也應該計算到預載入佇列中，但這樣就麻煩了，要退必須退整體，否則可能出現問題；
			$.webviews[id] = {
				webview: webview, //目前僅preload的快取webview
				preload: true,
				show: $.showOptions(options.show),
				afterShowMethodName: options.afterShowMethodName //就不應該用evalJS。應該是通過事件訊息通訊
			};
			//索引該預載入視窗
	...
	$.preload = function(options) {
		//呼叫預載入函式，不管是否傳遞preload參數，強制變為true
		if(!options.preload) {
	...
	$.appendWebview = function(options) {
		...
		if(!$.webviews[id]) { //保證執行一遍
			//TODO 這裡也有隱患，比如某個webview不是作為subpage建立的，而是作為target webview的話；
			if(!plus.webview.getWebviewById(id)) {
				webview = plus.webview.create(options.url, id, options.styles, options.extras);
			}
			//之前的實現方案：子視窗loaded之後再append到父視窗中；
			//問題：部分子視窗loaded事件發生較晚，此時執行父視窗的children方法會返回空，導致父子通訊失敗；
			//     比如父頁面執行完preload事件后，需觸發子頁面的preload事件，此時未append的話，就無法觸發；
	...
	$.addInit({
		...
		handle: function() {
			...
			if($.os.plus) {
				$.plusReady(function() {
					//TODO  這裡需要判斷一下，最好等子視窗載入完畢后，再呼叫主視窗的show方法；
					...
					if(plus.webview.currentWebview() === plus.webview.getWebviewById(plus.runtime.appid)) {
						//首頁需要自己啟用預載入；
						//timeout因為子頁面loaded之後才append的，防止子頁面尚未append、從而導致其preload未觸發的問題；
						setTimeout(function() {
					...
					if($.os.android && parseFloat($.os.version) < 4.4) {
						//解決Android平臺4.4版本以下，resume后，父窗體標題延遲渲染的問題；
				...
				//					span.innerHTML = '在該瀏覽器下，不支援建立子頁面，具體參考';
...
(function($, window) {
		...
		handle: function() {
			...
			if (parent) {
				...
				wobj.canBack(function(e) {
					//by chb 暫時註釋，在碰到類似popover之類的錨點的時候，需多次點選才能返回；
					if (e.canBack) { //webview history back
						window.history.back();
					} else { //webview close or hide
						//fixed by fxy 此處不應該用opener判斷，因為使用者有可能自己close掉目前視窗的opener。這樣的話。opener就為空了，導致不能執行close
	...
	$.menu = function() {
		...
		if (menu) {
			$.trigger(menu, $.EVENT_START); //臨時處理menu無touchstart的話，找不到目前targets的問題
			$.trigger(menu, 'tap');
		} else { //執行父視窗的menu
			if (window.plus) {
			...
			if(hasPulldown || hasPullup) {
				...
				if(container) {
					...
					if($container.length === 1) {
						if($.os.plus) { //5+環境
							if(hasPulldown && pullRefreshOptions.down.style == "circle") { //原生轉圈
								$.plusReady(function() {
									//這裡改寫$.fn.pullRefresh
							...
							} else if($.os.android) { //非原生轉圈，但是Android環境
								$.plusReady(function() {
									//這裡改寫$.fn.pullRefresh
									$.fn.pullRefresh = $.fn.pullRefresh_native
									var webview = plus.webview.currentWebview();
									if(window.__NWin_Enable__ === false) { //不支援多webview
										$container.pullRefresh(pullRefreshOptions);
									} else {
										if(hasPullup) {
											//目前頁面初始化pullup
										...
										if(hasPulldown) {
											...
											if(parent) {
												if(!hasPullup) { //如果沒有上拉載入，需要手動初始化一個預設的pullRefresh，以便當前頁面容器可以呼叫endPulldownToRefresh等方法
	...
	$.plusReady(function() {
		$.ajaxSettings = $.extend($.ajaxSettings, {
			xhr: function(settings) {
				if (settings.crossDomain) { //強制使用plus跨域
					return new plus.net.XMLHttpRequest();
				}
				//僅在webview的url為遠端檔案，且ajax請求的資源不同源下使用plus.net.XMLHttpRequest
				...
				if ($.os.ios && window.webkit && window.webkit.messageHandlers) { //wkwebview下同樣使用5+ xhr
                    return new plus.net.XMLHttpRequest();
                }
				return new window.XMLHttpRequest();
			}
	...
	var Scroll = $.Class.extend({
		...
		_drag: function(e) {
			...
			if (this.options.scrollY || detail.direction === 'up' || detail.direction === 'down') { //如果是豎向滾動或手勢方向是上或下
				//ios8 hack
				if ($.os.ios && parseFloat($.os.version) >= 8) { //多webview時，離開目前webview會導致後續touch事件不觸發
			...
			if (this.moved || isPreventDefault) {
				e.stopPropagation(); //阻止冒泡(scroll類巢狀)
			...
			if (!this.moved) {
				...
				e.stopPropagation(); //move期間阻止冒泡(scroll巢狀)
			}
			var deltaX = 0;
			var deltaY = 0;
			if (!this.moved) { //start
		...
		_transitionTime: function(time) {
			...
			if (this.parallaxElement && this.options.scrollY) { //目前僅支援豎向視差效果
		...
		_reLayout: function() {
			...
			if (this.options.snap && typeof this.options.snap === 'string') {
				...
				this._initSnap(); //需要每次都_initSnap么。其實init的時候執行一次，後續resize的時候執行一次就行了吧.先這麼做吧，如果影響效能，再調整
			}
		},
...
(function($, window, document, undefined) {
    ...
    var PullRefresh = $.Scroll.extend($.extend({
        ...
        _drag: function(e) {
            if (this.y >= 0 && this.disablePulldown && e.detail.direction === 'down') { //禁用下拉重新整理
                return;
            }
            this._super(e);
        ...
        pulldownLoading: function(y, time) {
            typeof y === 'undefined' && (y = this.options.down.height); //預設高度
    ...
    $.fn.pullRefresh = function(options) {
        if (this.length === 1) {
            ...
            if (options.down && options.down.auto) { //如果設定了auto，則自動下拉一次
                pullRefreshApi.pulldownLoading(options.down.autoY);
            } else if (options.up && options.up.auto) { //如果設定了auto，則自動上拉一次
...
(function($, window) {
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		handleEvent: function(e) {
			...
			switch (e.type) {
				...
				case $.eventName('shown', 'tab'):
					if (~this.snaps.indexOf(e.target)) { //避免巢狀監聽錯誤的tab show
		...
		_gotoItem: function(slideNumber, time) {
			this.currentPage = this._getPage(slideNumber, true); //此處傳true。可保證程式切換時，動畫與人手操作一致(第一張，最後一張的切換動畫)
 ...
 */
(function($, document) {
    if (!($.os.plus)) { //僅在5+android支援多webview的使用
        return;
    }
    $.plusReady(function() {
        if (window.__NWin_Enable__ === false) { //不支援多webview，則不用5+下拉重新整理
            return;
        }
        ...
        var PlusPullRefresh = $.Class.extend({
                ...
                self.scrollInterval = window.setInterval(function() {
                    if (self.isScroll && !self.loading) {
                        if (window.pageYOffset + window.innerHeight + 10 >= document.documentElement.scrollHeight) {
                            self.isScroll = false; //放在這裡是因為快速滾動的話，有可能檢測時，還沒到底，所以只要有滾動，沒到底之前一直檢測高度變化
            ...
            _initPulldownRefreshEvent: function() {
                ...
                $.plusReady(function() {
                    if (self.options.down.style == "circle") {
                        ...
                        var webview = plus.webview.getWebviewById(self.options.webviewId); //子視窗
                        if (!webview) {
                            return;
                        }
                        ...
                        webview.addEventListener('close', function() {
                            var attrWebviewId = self.options.webviewId && self.options.webviewId.replace(/\//g, "_"); //替換所有"/" 
                        ...
                        webview.addEventListener("dragBounce", function(e) {
                            ...
                            switch (e.status) {
                                case "beforeChangeOffset": //下拉可重新整理狀態
                                    self._setCaption(downOptions.contentdown);
                                    break;
                                case "afterChangeOffset": //鬆開可重新整理狀態
                                    self._setCaption(downOptions.contentover);
                                    break;
                                case "dragEndAfterChangeOffset": //正在重新整理狀態
                                    //執行下拉重新整理所在webview的回撥函式
            ...
            setStopped: function(stopped) { //該方法是子頁面呼叫的
                this.stopped = !!stopped;
                // TODO 此處需要設定目前webview的bounce為none,目前5+有BUG
                if (this.stopped) {
                    this.disablePullupToRefresh();
                    this.disablePulldownToRefresh();
                } else {
            ...
            beginPulldown: function() {
                var self = this;
                $.plusReady(function() {
                    //這裡延時的目的是爲了保證下拉重新整理元件初始化完成，後續應該做成有狀態的
                    setTimeout(function() {
                        if (self.options.down.style == "circle") { //單webview下拉重新整理
            ...
            pulldownLoading: function() { //該方法是子頁面呼叫的，相容老的歷史API
                this.beginPulldown();
            },
            _pulldownLoading: function() { //該方法是父頁面呼叫的
                var self = this;
                $.plusReady(function() {
            ...
            endPulldownToRefresh: function() { //該方法是子頁面呼叫的，相容老的歷史API
                this.endPulldown();
            },
            _endPulldownToRefresh: function() { //該方法是父頁面呼叫的
                var self = this;
                if (self.topPocket && self.options.webview) {
                    self.options.webview.endPullToRefresh(); //下拉重新整理所在webview回彈
                    self.loading = false;
                    self._setCaption(self.options.down.contentdown, true);
                    setTimeout(function() {
            ...
            beginPullup: function(callback) { //開始上拉載入
                var self = this;
                ...
                setTimeout(function() {
                    ...
                    self.pullCaption.innerHTML = ''; //修正5+裡邊第一次載入時，文字顯示的bug(還會顯示出來個「多」,猜測應該是渲染問題導致的)
            ...
            pullupLoading: function(callback) { //相容老的API
                this.beginPullup(callback);
            },
            endPullup: function(finished) { //上拉載入結束
                var self = this;
                if (self.pullLoading) {
            ...
            endPullupToRefresh: function(finished) { //上拉載入結束，相容老的API
                this.endPullup(finished);
            },
            disablePulldownToRefresh: function() {
                var webview = plus.webview.currentWebview();
                if (this.options.down.style && this.options.down.style == 'circle') { // 單webview模式禁止原生下拉重新整理
                    this.options.webview.setPullToRefresh({
                        support: false,
                        style: 'circle'
                    });
                } else { // 雙webview模式禁止下拉重新整理
                    webview.setStyle({
                        bounce: 'none'
                    });
                    webview.setBounce({
...
(function($, window, document, name) {
	...
	window.addEventListener('tap', function(e) {
		...
		//TODO 此處型別的程式碼後續考慮統一優化(target機制)，現在的實現費力不討好
		var target = e.target;
		for (; target && target !== document; target = target.parentNode) {
...
(function($, window, document, name, undefined) {
   ...
   var handle = function(event, target) {
        if (target.classList && (target.classList.contains(CLASS_CONTROL_ITEM) || target.classList.contains(CLASS_TAB_ITEM))) {
            if (target.parentNode && target.parentNode.classList && target.parentNode.classList.contains(CLASS_SEGMENTED_CONTROL_VERTICAL)) {
                //vertical 如果preventDefault會導致無法滾動
                    ...
                    event.preventDefault();      
                    // if(target.tagName == 'A') {
                    //     // fixed 底部選項卡href 無法跳轉 && stop hash change
    ...
    window.addEventListener('tap', function(e) {
        ...
        for (var i = 0; i < _contents.length; i++) { //查詢直屬子節點
...
(function($, window, document) {
	...
	window.addEventListener($.EVENT_END, function(event) { //使用touchend來取消高亮，避免一次點選既不觸發tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
	...
	window.addEventListener($.EVENT_CANCEL, function(event) { //使用touchcancel來取消高亮，避免一次點選既不觸發tap，doubletap，longtap的事件
		if (!cell) {
			return;
		}
		toggleActive(false);
        ...
        var popup = {
            ...
            close: function(index, animate) {
                if (popupElement) {
                    ...
                    if (result === false) { //返回false則不關閉目前popup
                        return;
                    }
                    if (animate !== false) {
...
(function($, document) {
	...
	/**
	 * 建立並顯示進度條 
	 * @param {Object} container  可選，預設body，支援selector,DOM Node,mui wrapper
	 ...
	 * 設定指定進度條進度 
	 * @param {Object} container  可選，預設body，支援selector,DOM Node,mui wrapper
	 * @param {Object} progress 可選，預設0 取值範圍[0-100]
...
(function($, window, document) {
	...
	Input.prototype.initElementEvent = function() {
		...
		if (this.sliderActionClass) {
			...
			var timer = null;
			var showTip = function() { //每次重新計算是因為控制元件可能被隱藏，初始化時計算是不正確的
...
(function($, window) {
    ...
    var Transparent = function(element, options) {
        this.element = element;
        this.options = $.extend({
            top: 0, //距離頂部高度(到達該高度即觸發)
            offset: 150, //滾動透明距離
            duration: 16, //過渡時間
            scrollby: window//監聽滾動距離容器
        }, options || {});

        this.scrollByElem = this.options.scrollby || window;
        if (!this.scrollByElem) {
            throw new Error("監聽滾動的元素不存在");
        }
...
(function($, window, document) {
    ...
    var defaultOptions = {
        ...
        loadingIcon: 'mui-spinner' + ' ' + 'mui-spinner-white', //圖示，可為空
        loadingIconPosition: 'left' //圖示所處位置，僅支援left|right
    };
